    ...
    return (
        <button
            ...
            className={cn(
                unstyled
                    ? ''
                    : 'flex w-full items-center gap-2 rounded-full border-0 border-white/12 bg-surface-0 px-4 py-3 text-surface-950 shadow-stroke transition-all hover:bg-surface-100 group-data-[open=true]:bg-surface-100/75 [&>span]:truncate',
                className
            )}
            {...props}
        />
    ...
    return (
        <div
            ...
            className={cn(
                ...
                unstyled ? '' : 'rounded-lg border-0 border-white/12 bg-surface-0 p-1 shadow-stroke',
...
const DropdownMenuItem = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(({ className, onClick, ...props }, ref) => {
    ...
    return <button ref={ref} onClick={handleClick} className={cn(unstyled ? '' : 'flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left hover:bg-surface-100 [&>span]:truncate', className)} {...props} />;
});
